# Build it
cd model-init
sudo podman build -t quay.io/rh-ee-soanders/mangey-moose-model-init:v1 .

# Run it with the host models dir mounted
sudo podman run --rm \
  --device nvidia.com/gpu=all \
  -v /var/lib/mangey-moose/models:/models:z \
  quay.io/rh-ee-soanders/mangey-moose-model-init:v1
...
sudo podman run --rm \
  --device nvidia.com/gpu=all \
  -v /var/lib/mangey-moose/models:/models:z \
  quay.io/ryh-ee-soanders/mangey-moose-model-init:v1
